...
function Card({ title, pdfUrl }) {
  ...
  return (
    <>
      <div class="h-35 cursor-pointer hover:bg-neutral-100 bg-white rounded-b-lg border-t-8 border-green-400 px-4 py-5 flex flex-col justify-around shadow-md" onClick={() => setIsModalOpen(true)}>
        <p class="text-base font-bold font-sans" title={title}>{truncateTitle(title)}</p>
        <div class="flex justify-between">
          <Icon icon="material-symbols-light:book-ribbon-rounded" width="30" height="30" className="text-neutral-400" />
          <div class="text-sm flex gap-2">
            <button class="bg-slate-200 px-5 rounded-xl cursor-pointer hover:bg-slate-400 transition-colors ease-in-out" onClick={() => setIsModalOpen(true)}>
              View
            </button>
          </div>
        </div>
